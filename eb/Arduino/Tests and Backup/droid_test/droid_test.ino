...
void loop() {
  ...
  // Buttons are single byte with the button number. IMU is 4 bytes: [button_number = 255], [Pitch] [Roll] [Compass]
  ...
  while (Serial1.available() > 0) {  // Checks whether data is comming from the serial port
    ...
    if (button_number == 255) {
      // Read IMU data (in Degrees)
      Serial.println("GOT ACC START");
      int pitch = Serial1.read();
      Serial.print("Pitch = ");     // level is 90 degrees, straight up is zero degrees
      Serial.println(pitch, DEC);   // Arduino local console       
      int roll = Serial1.read();
      Serial.print("Roll = ");
      Serial.println(roll, DEC);    // Arduino local console       
      int azimuth = Serial1.read(); // Compass
      azimuth = azimuth * 10;       // Sent as 1/10 value to fit in a byte (and accuracy not important)
      Serial.print("Compass = ");
      Serial.println(azimuth, DEC); // Arduino local console       
