...
void bt_serial_read() { // look for 4 byte messages from bluetooth phone (start byte + 3 data bytes)
  while (Serial1.available() > 0) {  // Checks whether data is coming from the serial port
    int new_byte = Serial1.read();       // Reads the data from the serial port

    if (msg_bytes_received == 0) {
      // looking for start byte
      if (new_byte == 0xFF) {        
        msg_bytes_received++; // got start byte
      }
    }
    elif (msg_bytes_received == 1) {

    }
    elif (msg_bytes_received == 2) {

    }
    elif (msg_bytes_received == 3) {
