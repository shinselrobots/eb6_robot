...
class WalkEngine():
    ...
    def update_ankle_servos(self, speed_cmd):
        ...
        pub_right_leg_ankle_rotate.publish(BAL_ANKLE_NEUTRAL_POSITION + (speed_cmd / 15.0))
        pub_left_leg_ankle_rotate.publish( BAL_ANKLE_NEUTRAL_POSITION + (speed_cmd / 15.0))
    

    def motor_ramp(self):
        # called each time through the fast loop. Speed command is -1.0 to +1.0
        # Phase 1: ramp up the ankle servos
        #self.bal_motor_command
        ...
        (self.bal_joy_speed_cmd, self.bal_speed_ramp, self.imu_pitch, self.braking_overshoot, self.bal_motor_command))
    ...
    def joy_callback(self, data):
        ...
        if BAL_PID_MODE:
            if not top_right_trigger:
                ...
                # Motors enabled
                self.bal_joy_speed_cmd =  data.axes[3] # +/1 1.0 max  
            ...
            if BAL_DOUBLE_WHEEL:
                ...
                #if self.enable_balancing and not self.robot_fall_detected:
                #    pub_right_leg_ankle_rotate.publish(BAL_ANKLE_NEUTRAL_POSITION + (self.bal_joy_speed_cmd / 10.0))
                #    pub_left_leg_ankle_rotate.publish( BAL_ANKLE_NEUTRAL_POSITION + (self.bal_joy_speed_cmd / 10.0))
                pass
    ...
    def timer_loop(self, event=None):
        ...
        if (BAL_PID_MODE):
            ...
            self.bal_target_angle = self.motor_ramp()
