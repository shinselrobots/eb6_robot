...
BAL_DEFAULT_IMU_OFFSET = 8.0 #10.4 # 3.0
#BAL_ANKLE_NEUTRAL_POSITION = -0.70 # Position servo for robot not moving TODO fix this for all poses!
BAL_ANKLE_NEUTRAL_POSITION = -0.72 #TODO Head UP
...
class WalkEngine():
    def __init__(self):
        ...
        self.bal_joy_speed = 0.0
        self.bal_joy_turn = 0.0
    ...
    def joy_callback(self, data):
        ...
        if BAL_PID_MODE:
            if not top_right_trigger:
                ...
                self.bal_joy_turn = 0.0
                if BAL_DOUBLE_WHEEL:
                    pub_right_leg_ankle_rotate.publish(BAL_ANKLE_NEUTRAL_POSITION)
                    pub_left_leg_ankle_rotate.publish( BAL_ANKLE_NEUTRAL_POSITION)
            else:    
                # Motors enabled
                self.bal_joy_speed =  data.axes[3] # +/1 1.0 max  
                self.bal_joy_turn = data.axes[2]
            ...
            if BAL_DOUBLE_WHEEL:
                ...
                if self.enable_balancing and not self.robot_fall_detected:
                    pub_right_leg_ankle_rotate.publish(BAL_ANKLE_NEUTRAL_POSITION + (self.bal_joy_speed / 10.0))
                    pub_left_leg_ankle_rotate.publish( BAL_ANKLE_NEUTRAL_POSITION + (self.bal_joy_speed / 10.0))
                    pass
            ...
            if not top_right_trigger:
                ...
                forward_speed = data.axes[3] * DYNAMIXEL_64_MAX_SPEED
                turn_speed = data.axes[2] * DYNAMIXEL_64_MAX_SPEED
                ...
                # Calculate motor speeds, clamping to +/- 100 percent
                #right_speed = self.clamp((forward_speed - turn_speed), 100)  
                #left_speed = self.clamp((forward_speed + turn_speed), 100)
                
                # DEBUG TEST
                #right_speed = int(data.axes[3] * 100)  # right joystick vertical
                #left_speed = int(data.axes[1] * 100)  # left joystick vertical
    ...
    def set_walk_mode(self, mode):
        if mode == MODE_POSE0:    
            print("WALK MODE set to POSE0")
            self.walking_enabled = False
            
            if (BAL_PID_MODE):
                self.imu_pitch_hardware_offset_degrees = BAL_DEFAULT_IMU_OFFSET  # -17.0
        
            if DEBUG_HEAD_DOWN:  
                self.set_servo_speed(0.5) # Set all servos speed    
                pub_neck_raise.publish(-1.50)
                pub_head_tilt.publish(-1.47)
                pub_head_sidetilt.publish(0.00)
                pub_right_antenna.publish(1.50)
                pub_left_antenna.publish(1.50)
               
                pub_right_leg_thigh_lift.publish(-0.20)
                pub_left_leg_thigh_lift.publish(-0.20)
                pub_right_leg_knee_bend.publish(0.40)
                pub_left_leg_knee_bend.publish(0.40)
                #pub_right_leg_ankle_rotate.publish(-0.23)
                #pub_left_leg_ankle_rotate.publish(-0.23)
                
            else:        
                self.goal_pose = 1 # DAVE TODO FIX THIS OVERRIDING FROM POSE 0 TO POSE 1 FOR TESTING!
                slowest_servo_speed = 0.3
                self.robotpose.move(self.goal_pose, slowest_servo_speed)
                
                # REMOVE TORQUE AFTER MOVING TO POSE 0
                # rospy.sleep(2.0)
                # SetServoTorque(0.0, all_servo_joints) # 1.0 = max torque   all_non_wheel_joints?
        
        elif mode == MODE_POSE4:   
            print("WALK MODE set to POSE4")
            self.walking_enabled = False
            ...
            if DEBUG_HEAD_DOWN:
                print("WALK MODE set to POSE4 - HEAD DOWN ***********************************")
                self.set_servo_speed(0.5) # Set servos speed
                
                self.goal_pose = 4 # DAVE TODO FIX THIS KLUDGE!!!
                slowest_servo_speed = 0.3
                self.robotpose.move(self.goal_pose, slowest_servo_speed)
                ...
                pub_right_antenna.publish(1.50)
                pub_left_antenna.publish(1.50)
                pub_right_leg_thigh_lift.publish(-0.80)
                pub_left_leg_thigh_lift.publish(-0.80)
    ...
    def timer_loop(self, event=None):
        ...
        if (BAL_PID_MODE):
            ...
            # TODO first ramp anlke servos, then when at speed, reduce ankle slope and replace with bal_target_angle offset
            
            
            max_angle = 6.0 # degrees lean for full speed
            self.bal_target_angle = 0 # self.bal_joy_speed * max_angle
            #right_speed = self.clamp((forward_speed + turn_speed), 1.0)  
            #left_speed = self.clamp((forward_speed - turn_speed), 1.0)
            
            self.bal_current_angle = self.imu_pitch         
            current_error = self.bal_current_angle - self.bal_target_angle
            
            # For EB with double wheels, add dead zone to avoid hunting near zero
            current_error = self.deadband(current_error, self.slider04/10) # 1.2
            
            tmp_sum = self.bal_error_sum + current_error
            self.bal_error_sum = self.constrain(tmp_sum, -200.0, 200.0) # Limit correction history
            ...
            motor_command = (pcalc + icalc + dcalc) / 10.0
            # prevent slow drift
            motor_command = self.deadband(motor_command, self.slider03/10) # 0.4

            
            #motor_command = (self.bal_kp * current_error) + (self.bal_ki * self.bal_error_sum * pid_sample_time) - (self.bal_kd * (self.bal_current_angle-self.bal_prev_angle) / pid_sample_time)
            ...
            if True: # (self.currentMillis - self.previousStepMillis) > 18.0: # every n milliseconds
                # Don't flood servo queue with too many messages
                
                
                left_speed =  motor_command # + (self.bal_joy_speed * 5.0)
                right_speed = motor_command # + (self.bal_joy_speed * 5.0)
                ...
                if current_error != 0.0:
                    rospy.loginfo("Pc:% 2.2f Ic:% 2.2f Dc:% 2.2f Err:% 2.1f ErrSum:% 2.1f M:% 2.1f | P:% 2.2f I:% 2.2f D:% 2.2f | IMU:%2.1f Time:% 2.1f" % 
                ...
                if (self.bal_current_angle < -40.0) or (self.bal_current_angle > 40.0):
                    ...
                    # SEND WHEEL COMMANDS!
                    turn = self.bal_joy_turn * 0.5 # Dynamixel is 6.2 max 
        ...
        if WHEEL_MOTOR_MODE:
            ...
            return # Default - do nothing. Handled in joystick handler.
            
            # For motor tuning / testing, run motor forward and backward:
            
            test_rate = self.slider12 * 10
            abs_speed = self.slider02 / 10
            elapsed_time = self.currentMillis - self.previousStepMillis
            if test_rate <= 0:
                self.pub_wheel_cmd_right.publish(0) # reversed motor
                self.pub_wheel_cmd_left.publish(0) 
            
            elif (self.currentMillis - self.previousStepMillis) > test_rate: # every n milliseconds
        
                # Calculate motor speeds, clamping to +/- 6.0 for Dynamixel MX28
                # TODO THIS IS A TEST:
                motor_speed = 0
                if self.dbg_flipflop:
                    self.dbg_flipflop = False
                    motor_speed = abs_speed           
                else:
                    self.dbg_flipflop = True
                    motor_speed = -abs_speed
                print("FF Motor speed: %d, elapsed time: % 2.1f" % (motor_speed, elapsed_time))
                
                self.pub_wheel_cmd_right.publish(-motor_speed) # reversed motor
                #self.pub_wheel_cmd_left.publish(motor_speed) 
